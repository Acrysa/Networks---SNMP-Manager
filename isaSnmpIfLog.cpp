...
int main(int argc, char* argv[])
 {
	...
	return 0;
     ...
     strftime(date_print,20,"%Y-%m-%d %T.",localtime(&date.tv_sec));
     printf("%s%ld ",date_print,date.tv_usec);
     
     interface = received_msg[41];
	...
	{          
	   ...
	   if(val_size == 0)
		...
		cout<<" ;";
		 ...
		 {
		    cout<<received_msg[38 + com_str.size() + m];
		 }
		...
		for(int n=1; n<=val_size; n++)
		{
		   ...
		   val_print += received_msg[38 + com_str.size() + n];	   
		}

		if(par_num == 6)
		{
		   cout<<hex<<val_print<<";"<<dec;
		}
		if(par_num == 22)
		{
		   cout<<"."<<val_print;
...
int send(char packet)
 {
     ...
     // Set a timeout for waiting for response, we won't wait forever!
